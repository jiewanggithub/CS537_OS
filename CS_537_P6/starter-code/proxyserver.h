...
char* http_get_mime_type(char* filename);
...
char* http_get_mime_type(char* filename){
    ...
    if(path_len < 5 || filename[path_len - 1] != 'l' || filename[path_len - 2] != 'm' || filename[path_len - 3] != 't' || filename[path_len - 4] != 'h' || filename[path_len - 5] != '.'){
       return "text/plain";
    }
    else{
        return "text/html";
